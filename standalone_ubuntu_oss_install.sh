...
"S3_REGION" "S3_STYLE" "ALLOW_DIRECTORY_LIST" "AWS_SIGS_VERSION" "LAMBDA_SERVER_PROTO" "LAMBDA_SERVER" "LAMBDA_SERVER_PORT" "LAMBDA_REGION")
...
if [ "${LAMBDA_SERVER_PROTO}" != "http" ] && [ "${LAMBDA_SERVER_PROTO}" != "https" ]; then
    >&2 echo "LAMBDA_SERVER_PROTO contains an invalid value (${LAMBDA_SERVER_PROTO}). Valid values: http, https"
    failed=1
fi
...
LAMBDA_SERVER_PROTO=${LAMBDA_SERVER_PROTO}
LAMBDA_SERVER=${LAMBDA_SERVER}
LAMBDA_SERVER_PORT=${LAMBDA_SERVER_PORT}
...
function download() {
  wget --quiet --output-document="$2" "https://raw.githubusercontent.com/shawnginx/nginx-s3-gateway/master/$1"
}
